      ...
      <div class="crumbs">
        ...
        <div class="cantainer">
          <el-form :inline="true" class="demo-form-inline" style="float: left;padding-bottom: 10px;">
           ...
           <!-- 	<input type="text" name="" id="" placeholder="搜索" v-model="search"/> -->
            ...
            <el-button type="primary">
            	<download-excel class="export-btn" :data="tableList" :fields="jsonFields" type="xls" header="房主信息" name="房主信息.xls">
          ...
          <el-table style="width: 100%;" border header-align="center"
            ...
            <el-table-column label="房主编号" prop="houserId"  min-width="100px" align="center">
              <template slot-scope="{row,$index}">
                <span>{{row.houserId}}</span>
              </template>
            </el-table-column>
            <el-table-column label="房主姓名" prop="houserName" min-width="100px" align="center">
              <template slot-scope="{row,$index}">
                <input class="edit-cell" v-if="showEdit[$index]" v-model="row.houserName">
                <span v-if="!showEdit[$index]">{{row.houserName}}</span>
              </template>
            </el-table-column>
            <el-table-column label="性别" prop="houserSex" min-width="88px" align="center">
              <template slot-scope="{row,$index}">
                <el-radio-group v-if="showEdit[$index]" v-model="row.houserSex">
                  <el-radio label="男"></el-radio>
                  <el-radio label="女"></el-radio>
                </el-radio-group>
                <span v-if="!showEdit[$index]">{{row.houserSex}}</span>
              </template>
            </el-table-column>
            <el-table-column label="电话号码" prop="houserPhone" min-width="100px" align="center">
              <template slot-scope="{row,$index}">
                <input class="edit-cell" v-if="showEdit[$index]" v-model="row.houserPhone">
                <span v-if="!showEdit[$index]">{{row.houserPhone}}</span>
              </template>
            </el-table-column>
            <el-table-column label="电话号码" prop="houserWages" min-width="100px" align="center">
              <template slot-scope="{row,$index}">
                <input class="edit-cell" v-if="showEdit[$index]" v-model="row.houserWages">
                <span v-if="!showEdit[$index]">{{row.houserWages}}</span>
              </template>
            </el-table-column>
            <el-table-column label="备注" prop="houserRemarks" min-width="150px" align="center">
              <template slot-scope="{row,$index}">
                <el-popover trigger="hover" placement="top">
                  <input class="edit-cell" v-if="showEdit[$index]" v-model="row.houserRemarks">
                  <span v-if="!showEdit[$index]">{{row.houserRemarks}}</span>
                  <div slot="reference">
                  <span style="
...
/*      btn:function(){
      ...
      /* 获取云端数据 */
      handleUserList() {
        this.$http.get('http://localhost:8945/houser/All').then(res => { //这是从本地请求的数据接口，
      ...
      handleUpdate(index, row) {
        ...
        this.$confirm('确定修改当前数据?', '提示', {
          ...
          this.axios({
            method: "put",
            url: "http://localhost:8945/houser",
            headers: {
      ...
      deleteData(index, row) {
        ...
        this.$confirm('确定删除该数据?', '提示', {
          ...
          this.axios.delete('http://localhost:8945/houser/' + row.houserId).then((response) => {
